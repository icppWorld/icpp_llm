...
// ICPP: no need for mmap to get data from files.
//       we already have all bytes uploaded & store in a
//       std::vector<uint8_t> that is orthogonally persisted
//
// #if defined _WIN32
// #include "win.h"
// #else
...
bool bpe_encode(const char *text, char **vocab, float *vocab_scores,
  ...
  char *str_buffer =
      malloc((max_token_length * 2 + 1) *
             sizeof(char)); // *2 for concat, +1 for null terminator
...
//   char *checkpoint = NULL;  // e.g. out/stories15M.bin
...
//   // read in the stories15M.bin file
//   Config config;
...
//   // ICPP: see the initialize_tokenizer() function in initialize.cpp
//   //
...
//     bpe_encode(prompt, vocab, vocab_scores, config.vocab_size, max_token_length,
